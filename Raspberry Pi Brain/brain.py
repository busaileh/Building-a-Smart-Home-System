...
from extras.SMS import sendTextMessage
...
def homeSecurityFunc(rc_data):
    ...
    global secure,lastSecure,rainy,window,door

    if(len(rc_data) > 0):

        print("Entered Home Security function")
        ...
        if(device == "dev2"):
            ...
            if(dataType =="security"):
                ...
                door = data
                if(door):
                    os.system("sudo ./extras/take_pics") #Take pics when door opens
                    now = timer()
